...
import { TrueForgeHarnessAdapter } from './trueforge'
...
describe('createHarnessAdapter', () => {
  ...
  it('constructs the live adapter only from a validated local configuration', () => {
    expect(createHarnessAdapter({
      mode: 'configured',
      baseUrl: 'http://127.0.0.1:8790',
      modelName: 'provider/model',
    })).toBeInstanceOf(TrueForgeHarnessAdapter)
  })
})
